...
/**
 * Sesle yazı bileşeni. Tüm cihazlarda (masaüstü + mobil) aynı akış:
 * MediaRecorder ile ses kaydı → sunucu tarafında LLM tabanlı transcription
 * (bkz. /api/transcribe, Gemini Flash Lite audio).
 */
export function VoiceInput({
  ...
  const [state, setState] = React.useState<VoiceState>({ kind: "idle" })
  ...
  const startRecording = React.useCallback(async () => {
    // /api/transcribe auth ister (aksi halde 401). Giriş yoksa kayda hiç
    // başlama; doğrudan login dialog'u aç — guard davranışını yansıt.
    if (!isAuthenticated) {
      loginDialog.open()
      return
    }
  ...
  }, [clearTimers, fullReset, stopStream, isAuthenticated])
  ...
  const confirmTranscribe = React.useCallback(async () => {
    ...
    if (res.status === 401) {
      // Oturum review sırasında düşmüş olabilir — sayfayı yenile demek yerine
      // doğrudan login dialog'u aç. Kayıt "stopped"ta kalır; giriş sonrası
      // kullanıcı tekrar İlet'e basabilir.
      loginDialog.open()
      setState({ kind: "stopped", audioBlob, mediaType, elapsedMs: stoppedElapsed })
      return
    }
